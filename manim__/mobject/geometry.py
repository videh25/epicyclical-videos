...
class Line(VMobject):
    ...
    def __init__(self, start, end = ORIGIN, **kwargs):
    ...
    def put_start_on(self, new_start):
...
class Arrow(Line):
    CONFIG = {
        "tip_length": 1,
        "tip_width_to_length_ratio": 0.35,
        "max_tip_length_to_length_ratio": 0.207,
        "max_stem_width_to_tip_width_ratio": 0.1,
        "buff": MED_SMALL_BUFF,
        "propagate_style_to_family": False,
        "preserve_tip_size_when_scaling": True,
        "normal_vector": OUT,
        "use_rectangular_stem": True,
        "rectangular_stem_width": .75,
        "stroke_width": 3,
    }
    ...
    def move_to(self,target):
        point_to_align = Line(self.points[0],self.points[-1]).get_center()
...
class Vector(Arrow):
    ...
    def __init__(self,vectip, vectail = ORIGIN,  **kwargs):
        if len(vectip) == 2 and len(vectail) == 2:
            direction = np.append(np.array(vectip-vectail), 0)
            self.tip_length = (np.sqrt((direction[0])**2 + direction[1]**2))*0.25
        Arrow.__init__(self, vectail, vectip, **kwargs)
...
'''class self_add_vec(Vector):
    def __init__(**kwargs):
        Arrow.__init__(**kwargs)
    '''
