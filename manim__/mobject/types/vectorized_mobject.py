...
class VMobject(Mobject):
    ...
    def reflect_vectorized_mobject(self):
        ...
        if self.get_num_points() == 0:
            ...
            for i in range(len(old_points)):
                new_points.append((-1)*old_points[i])
            new_path.start_at(new_points[0])
            new_path.add_control_points(new_points[1:])
            #new_path.set_stroke(self.stroke_color, self.stroke_width, self.stroke_opacity)
            return new_path
